...
// Получаем объявление и проверяем, что оно принадлежит текущему пользователю
...
if (isset($_POST['delete'])) {
    ...
    if ($stmt->execute([$ad_id, $_SESSION['user_id']])) {
        redirect('my_ads.php');
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("
            UPDATE ads 
            SET title = ?, animal_type = ?, status = ?, description = ?, location = ?, photo_path = ?, updated_at = CURRENT_TIMESTAMP, is_approved = FALSE
            WHERE id = ? AND user_id = ?
        ");
        
        if ($stmt->execute([$title, $animal_type, $status, $description, $location, $photo_path, $ad_id, $_SESSION['user_id']])) {
            $success = 'Объявление обновлено! Оно отправлено на повторную модерацию.';
            // Обновляем данные для отображения
            $ad['title'] = $title;
            $ad['animal_type'] = $animal_type;
            $ad['status'] = $status;
            $ad['description'] = $description;
            $ad['location'] = $location;
            $ad['photo_path'] = $photo_path;
        } else {
...
<head>
    <meta charset="UTF-8">
    <title>Редактирование объявления - HelpPaw</title>
            ...
            <div class="form-right-and-left">
                        ...
                        <div class="photo-upload" id="photoUpload">
                            ...
                            <div class="photo-preview">
                                <?php if (!empty($ad['photo_path'])): ?>
                                    <img src="<?= htmlspecialchars($ad['photo_path']) ?>" class="preview-image">
                                <?php else: ?>
                ...
                <div class="form-right">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select id="animal_type" name="animal_type" required>
                                ...
                                <option value="dog" <?= $ad['animal_type'] == 'dog' ? 'selected' : '' ?>>Собака</option>
                                <option value="cat" <?= $ad['animal_type'] == 'cat' ? 'selected' : '' ?>>Кошка</option>
                                <option value="other" <?= $ad['animal_type'] == 'other' ? 'selected' : '' ?>>Другое</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="status" name="status" required>
                                <option value="">Выберите</option>
                                <option value="lost" <?= $ad['status'] == 'lost' ? 'selected' : '' ?>>Потерялся</option>
                                <option value="found" <?= $ad['status'] == 'found' ? 'selected' : '' ?>>Нашёлся</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="location" name="location" value="<?= htmlspecialchars($ad['location']) ?>" placeholder="Город, район, ориентир">
                    </div>
                    
                    <div class="form-group">
                        <label for="description">Описание</label>
                        <textarea id="description" name="description" rows="5" required><?= htmlspecialchars($ad['description']) ?></textarea>
                    </div>
                </div>
            </div>
            
            <div class="form-buttons">
                <button type="submit" name="save" class="btn-submit">Сохранить изменения</button>
                <button type="submit" name="delete" class="btn-submit btn-delete" onclick="return confirm('Вы уверены, что хотите удалить это объявление? Действие необратимо.')">Удалить объявление</button>
            </div>
        </form>
    </main>
